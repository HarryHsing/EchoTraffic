...
class Video_Audio_Instruct_Dataset(BaseDataset):
    ...
    def collater(self, instances):
        ...
        if 'video_path' in instances[0]:
            video_path = [instance['video_path'] for instance in instances]
...
def preprocess_multimodal(
    ...
    # Zhxing
    is_multimodal = True
    image_token_len = cur_token_len + 8  # ✅ 加上音频的 8 个 token（你启用了 audio）

    patch_tokens = " ".join([DEFAULT_IMAGE_PATCH_TOKEN] * image_token_len)
    conversation_list[0]["q"] = f"<Video>{patch_tokens}</Video> {msg} {conversation_list[0]['q']}"
