...
class VideoLLAMA(Blip2Base):
    ...
    def __init__(
        ...
        for name, param in self.llama_model.named_parameters():
            param.requires_grad = not frozen_llama
        ...
        if enabled_lora:
            ...
            self.llama_model = get_peft_model(self.llama_model, lora_config).base_model.model

        self.frozen_llama = frozen_llama
        self.enabled_lora = enabled_lora
        self.dynamic_connector = dynamic_connector
        
        if frozen_llama and enabled_lora and dynamic_connector:
            self.multihead_attn = nn.MultiheadAttention(self.Qformer.config.hidden_size, 8)
            self.tokens_weight_proj = nn.Linear(self.Qformer.config.hidden_size, 1)
        ...
        self.equip_audio_branch = equip_audio_branch
        if equip_audio_branch:
            ...
            self.audio_encoder.load_state_dict(torch.load("{}".format(imagebind_ckpt_path)))
            # free vision encoder
    ...
    def encode_videoQformer_visual(self, image):
        ...
        with self.maybe_autocast():
            ...
            if self.frozen_llama and self.enabled_lora and self.dynamic_connector:
                video_hidden, _ = self.multihead_attn(video_hidden, video_hidden, video_hidden)
                tokens_weight = self.tokens_weight_proj(video_hidden)
                tokens_weight = F.softmax(tokens_weight, dim=1)
                inputs_llama = inputs_llama * tokens_weight
    ...
    def encode_videoQformer_audiovideo(self, image, audio, modality_type=ModalityType.AUDIO):
        device = image.device
        ...
        with self.maybe_autocast():
            ...
            video_frame_position_embeddings = frame_position_embeddings.unsqueeze(-2)
            frame_hidden_state = einops.rearrange(q_hidden_state, '(b t) q h -> b t q h',b=batch_size,t=time_length)
            frame_hidden_state = video_frame_position_embeddings + frame_hidden_state
            ...
            video_hidden = video_query_output.last_hidden_state
            inputs_llama_video = self.llama_proj(video_hidden)
            ...
            audio_query_output = self.audio_Qformer.bert(
                query_embeds=audio_query_tokens, #[8,768]
                encoder_hidden_states=audio_imagebind_finalout,
                encoder_attention_mask=frame_atts,
                return_dict=True,
                )
            audio_hidden = audio_query_output.last_hidden_state
            inputs_llama_audio = self.audio_llama_proj(audio_hidden)
    ...
    def forward(self, samples):
        if 'conv_type' in samples.keys() and samples['conv_type']=='multi':
            ...
            if not self.equip_audio_branch and image != None:
                num_patch_tokens = self.num_video_query_token
                img_embeds, atts_img = self.encode_videoQformer_visual(image)
            elif image == None and audio != None and self.equip_audio_branch :
                ...
                num_patch_tokens = self.num_video_query_token + self.num_audio_query_token
                img_embeds, atts_img = self.encode_videoQformer_audiovideo(image, audio, modality_type=ModalityType.AUDIO)
    ...
    def from_config(cls, cfg):
        ...
        frozen_llama = cfg.get("frozen_llama", True)
        ...
        enabled_lora = cfg.get("enabled_lora", False)
        dynamic_connector = cfg.get("dynamic_connector", False)
        ...
        model = cls(
            ...
            dynamic_connector=dynamic_connector,
            lora_r=lora_r,
            lora_alpha=lora_alpha,
            lora_dropout=lora_dropout
        )
